//spectres
const SPECTRE_TRIGGER_EXPLODE_DIST = 300
const SPECTRE_EXPLOSION_RADIUSFALLOFFMAX = 450
...
const SUICIDE_FAST_MOVESPEED = 1.85
...
function __SpectreSelfDestructInternal( spectre, results, state )
{
	...
	else
	{
		...
		if ( IsValid( results.inflictor ) && IsPlayer( results.inflictor ) )
			waitTime = 0.15//if the player shot this guy, then give this much time for a tell before exploding
		else
		{
